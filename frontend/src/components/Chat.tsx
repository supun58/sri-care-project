    ...
    const initChat = async () => {
      try {
        ...
        if (response.data.success && response.data.data) {
          ...
          const historyData = historyResponse.data && historyResponse.data.data ? historyResponse.data.data : null;
          const historyMessages = historyData && historyData.messages ? historyData.messages : (historyData || []);
          if (historyResponse.data.success && Array.isArray(historyMessages)) {
            const loadedMessages = historyMessages.map((msg: any) => ({
              id: msg.id.toString(),
          ...
          const wsUrl = wsEnv || 'ws://localhost:3100/api/chat';
          ...
          {messages.map((message) => (
            ...
            >
              <div className={'w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ' + (
                message.sender === 'user' ? 'bg-blue-600' : message.sender === 'agent' ? 'bg-green-600' : 'bg-gray-600'
              )}>
                {message.sender === 'user' ? (
              ...
              <div className={'flex flex-col max-w-[70%] ' + (message.sender === 'user' ? 'items-end' : 'items-start')}>
                ...
                <div className={'flex items-center gap-1 mt-1 text-xs text-blue-600 ' + (message.sender === 'user' ? 'flex-row-reverse' : '')}>
                  <span>
