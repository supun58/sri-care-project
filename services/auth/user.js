...
class User {
  ...
  static setResetToken(userId, tokenHash, expiresAt, callback) {
    const sql = `UPDATE users SET reset_token = ?, reset_expires = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`;
    db.run(sql, [tokenHash, expiresAt, userId], function(err) {
      if (err) {
        callback(err);
      } else {
        callback(null, { success: true });
